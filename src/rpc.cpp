...
int32_t IRpc::OnMessage(int64_t handle, const uint8_t* msg, uint32_t msg_len) {
...
int32_t IRpc::SendMessage(int64_t handle, const RpcHead& rpc_head,
    ...
    int32_t send_ret = SendV(handle, sizeof(msg_frag)/sizeof(*msg_frag), msg_frag, msg_frag_len, 0);
    if (send_ret != 0) {
