...
#ifndef PEBBLE_DR_PROTOCOL_BINARYPROTOCOL_TCC
#define PEBBLE_DR_PROTOCOL_BINARYPROTOCOL_TCC
...
uint32_t TBinaryProtocolT<Transport_>::readMessageBegin(std::string& name,
  ...
  if (sz < 0) {
    ...
    result += readI64(seqid);
  } else {
    if (this->strict_read_) {
      throw TProtocolException(TProtocolException::BAD_VERSION, "No version identifier... old protocol client in strict mode?");
    } else {
      // Handle pre-versioned input
      /* TODO: 老版本兼容，pebble不涉及，这部分应该去掉 */
      int8_t type;
...
#endif // PEBBLE_DR_PROTOCOL_BINARYPROTOCOL_TCC
