...
#ifndef PEBBLE_DR_PROTOCOL_JSONPROTOCOL_H
#define PEBBLE_DR_PROTOCOL_JSONPROTOCOL_H

#include "dr/protocol/virtual_protocol.h"
 ...
 * yet verified whether cxx:lexical_cast, which is doing that work for me in
...
class TJSONProtocol : public TVirtualProtocol<TJSONProtocol> {
 public:
  ...
  TJSONProtocol(cxx::shared_ptr<TTransport> ptrans);
 ...
 private:
  void pushContext(cxx::shared_ptr<TJSONContext> c);
 ...
 private:
  ...
  std::stack<cxx::shared_ptr<TJSONContext> > contexts_;
  cxx::shared_ptr<TJSONContext> context_;
...
class TJSONProtocolFactory : public TProtocolFactory {
 public:
  ...
  cxx::shared_ptr<TProtocol> getProtocol(cxx::shared_ptr<TTransport> trans) {
    return cxx::shared_ptr<TProtocol>(new TJSONProtocol(trans));
  }
};
...
template<typename PebbleStruct>
std::string PebbleJSONString(const PebbleStruct& ts) {
    using namespace pebble::dr::transport;
    using namespace pebble::dr::protocol;
    TMemoryBuffer* buffer = new TMemoryBuffer;
    cxx::shared_ptr<TTransport> trans(buffer);
...
#endif // PEBBLE_DR_PROTOCOL_JSONPROTOCOL_H
