...
#include <vector>
#include <string>
#include "dr/transport/virtual_transport.h"
...
public:
    ...
    uint32_t readAll(uint8_t* buf, uint32_t len) {
        ...
        return pebble::dr::transport::readAll(*this, buf, len);
    }
    ...
    /**
     * 支持块协议，以消息包为单位收发，接收到完整包后写入buff，供应用阻塞read
     * @return : 0-未收到包
     */
...
public:
    static const uint32_t defaultSize = 1024 * 1024 * 2; // 2M
    static const uint32_t maxSize     = 1024 * 1024 * 8; // 8M
    ...
    std::string getBufferAsString() {
        ...
        uint32_t sz;
        getBuffer(&buf, &sz);
        return std::string(reinterpret_cast<char*>(buf), (std::string::size_type)sz);
    }

    void appendBufferToString(std::string& str) { // NOLINT
        if (buffer_ == NULL) {
            return;
        }
        uint8_t* buf;
    ...
    uint32_t readAppendToString(std::string& str, uint32_t len); // NOLINT
...
#endif // PEBBLE_DR_TRANSPORT_TBUFFERTRANSPORT_H
